...
const failureCodes = [
  ...
  'release_provider_unavailable', 'release_recovery_required',
] as const;
...
export const RELEASE_RECORD_SCHEMAS: Readonly<Record<string, ReleaseRecordSchema>> = deepFreeze({
  ...
  RouteRecordV1: schema('RouteRecordV1', {
    routeId: stringField(true, 'identifier'), serviceId: stringField(true, 'identifier'), publicIdentity: jsonField(true), desiredActiveSlot: enumField(['blue', 'green'], true),
    observedActiveUpstream: jsonField(), priorActiveSlot: enumField(['blue', 'green']), candidateConfigArtifactId: stringField(), candidateConfigDigest: stringField(false, 'digest'),
    previousConfigArtifactId: stringField(), previousConfigDigest: stringField(false, 'digest'), validationJobId: stringField(), validationResult: jsonField(),
    loadRequestDigest: stringField(false, 'digest'), loadResponseDigest: stringField(false, 'digest'), activeConfigReadbackDigest: stringField(false, 'digest'),
    privateProbeResult: jsonField(), publicProbeResult: jsonField(), streamSettings: jsonField(true), cutoverAt: stringField(false, 'timestamp'), rollbackAt: stringField(false, 'timestamp'),
    state: enumField(ROUTE_STATES, true), sequence: integerField(true), ambiguity: jsonField(), error: structuredError(),
  }),
  ...
  MaintenanceRecordV1: schema('MaintenanceRecordV1', {
    maintenanceId: stringField(true, 'identifier'), ownerPrincipal: stringField(true, 'identifier'), idempotencyKey: stringField(true, 'identifier'), creationRequestDigest: stringField(true, 'digest'),
    maintenanceKind: enumField(['INVENTORY', 'PACKAGE_UPDATE', 'SOFT_REBOOT', 'FULL_REBOOT', 'LIVEPATCH', 'RECOVERY'], true), packagePlan: jsonField(), approvalEvidence: objectArray(),
    state: enumField(['REQUESTED', 'SIMULATING', 'AWAITING_APPROVAL', 'VERIFYING_DISPOSABLE', 'APPLYING', 'REBOOT_REQUIRED', 'VERIFYING_HOST', 'SUCCEEDED', 'FAILED', 'RECOVERY_REQUIRED', 'AMBIGUOUS'], true),
    sequence: integerField(true), activeJobIds: stringArray(true), allJobIds: stringArray(true), rebootRequired: booleanField(true), livepatchState: jsonField(true),
    preSnapshot: jsonField(true), postSnapshot: jsonField(), evidenceIndexId: stringField(), error: structuredError(), createdAt: stringField(true, 'timestamp'), updatedAt: stringField(true, 'timestamp'),
  }),
});
