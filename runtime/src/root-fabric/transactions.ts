...
export class RootEffectTransactionService {
  ...
  reconcileTransition(transactionIdValue: string, nextState: RootEffectState, classification: string, details: JsonObject, controllerId = 'root-reconciler'): RootEffectTransaction {
    const transactionId = identifier(transactionIdValue, 'transactionId');
    if (!ROOT_EFFECT_STATES.includes(nextState)) throw new RootFabricError('invalid_request', 'reconciliation next state is invalid');
    const current = this.read(transactionId);
    if (current.lifecycle.terminal && current.lifecycle.persistedState !== nextState) throw new RootFabricError('transaction_state_conflict', 'ordinary reconciliation cannot alter a terminal transaction');
    const occurredAt = this.now();
    const fencingToken = current.lease.fencingToken + 1;
    const request = sha256(canonicalize({ operation: 'babyx.root.effect.reconcile', transactionId, priorState: current.lifecycle.persistedState, nextState, classification, details, controllerId, fencingToken }));
    const lease = { controllerId, leaseOwner: current.ownerPrincipal.principalId, fencingToken, acquiredAt: occurredAt, renewedAt: occurredAt, expiresAt: new Date(Date.parse(occurredAt) + 60_000).toISOString(), predecessorLease: current.lease.controllerId, takeoverReason: 'startup reconciliation' };
    const error = nextState === 'AMBIGUOUS' || nextState === 'RECOVERY_REQUIRED' || nextState === 'FAILED' ? { code: classification, message: `reconciliation classified transaction as ${nextState}`, retryable: nextState === 'RECOVERY_REQUIRED', phase: 'reconciliation', redactedDetails: details } : current.error;
    const candidate = appendEvent(current, 'babyx.root.effect.reconcile', 'reconciliation', nextState, request, sha256(`reconcile:${transactionId}:${fencingToken}`), occurredAt, { lease, error }, { classification, controllerId, fencingToken }, sha256(canonicalize(details)));
    assertTransaction(candidate);
    this.records.put(transactionId, candidate);
