...
test('A-G public surface is exact, dispatcher-backed, duplicate-free and contains no arbitrary root shell', () => {
  assert.equal(OPERATION_CATALOG_VERSION, '3.2.0');
  assert.equal(OPERATION_DEFINITIONS.length, 220);
  ...
  assert.equal(root.length, 41);
  assert.equal(ROOT_FABRIC_OPERATION_NAMES.length, 30);
  assert.equal(new Set(root).size, 41);
  for (const operation of [...original, ...ROOT_FABRIC_OPERATION_NAMES]) assert.equal(root.includes(operation), true, operation);
  const forbiddenSuffixes = ['credential.', 'freeze.', 'kill', 'reconcile'];
