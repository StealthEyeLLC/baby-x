...
test('A-G public surface is exact, dispatcher-backed, duplicate-free and contains no arbitrary root shell', () => {
  ...
  assert.equal(OPERATION_DEFINITIONS.length, 216);
  ...
  assert.equal(root.length, 37);
  assert.equal(ROOT_FABRIC_OPERATION_NAMES.length, 26);
  assert.equal(new Set(root).size, 37);
  for (const operation of [...original, ...ROOT_FABRIC_OPERATION_NAMES]) assert.equal(root.includes(operation), true, operation);
  const forbiddenSuffixes = ['observation.', 'credential.', 'freeze.', 'kill', 'reconcile'];
