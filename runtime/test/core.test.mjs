...
test('describe exposes only implemented operations without duplicate authority paths', () => {
  ...
  try {
    ...
    assert.equal(rootOperations.some((item) => /babyx\.root\.__checkpoint_k_not_implemented__/u.test(item.operation)), false);
    ...
    assert.equal(description.operationCatalogVersion, '3.5.0');
