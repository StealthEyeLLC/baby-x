...
test('describe exposes only implemented operations without duplicate authority paths', () => {
  ...
  try {
    ...
    assert.equal(rootOperations.some((item) => /babyx\.root\.(?:freeze\.|kill$|reconcile$)/u.test(item.operation)), false);
    ...
    assert.equal(description.operationCatalogVersion, '3.3.0');
