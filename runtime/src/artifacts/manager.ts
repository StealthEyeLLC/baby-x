...
import { closeSync, existsSync, mkdirSync, openSync, readFileSync, readSync, renameSync, rmSync, statSync, writeSync } from 'node:fs';
import { join } from 'node:path';
import { AtomicStore, canonicalize, type JsonObject } from '../core.ts';
...
export class ArtifactManager {
  ...
  create(name: string, sourcePath: string, metadata: JsonObject = {}): JsonObject { return this.createBytes(name, readFileSync(sourcePath), metadata); }
  createBytes(name: string, bytes: Buffer, metadata: JsonObject = {}): JsonObject { const record = this.begin(name, metadata); this.upload(String(record.id), 0, bytes); return this.finalize(String(record.id), bytes.length, createHash('sha256').update(bytes).digest('hex')); }
  createOnce(idempotencyKey: string, name: string, sourcePath: string, metadata: JsonObject = {}): JsonObject { return this.createBytesOnce(idempotencyKey, name, readFileSync(sourcePath), metadata); }
  ...
  verify(id: string): JsonObject {
    ...
    if (record.state !== 'finalized' || typeof record.path !== 'string' || typeof record.size !== 'number' || typeof record.sha256 !== 'string' || !existsSync(record.path)) throw new Error('artifact is not finalized');
    const bytes = readFileSync(record.path);
    const digest = createHash('sha256').update(bytes).digest('hex');
    if (bytes.length !== record.size || digest !== record.sha256) throw new Error('artifact integrity mismatch');
