...
export class RootRecoveryService {
  ...
  async kill(payloadValue: unknown, context: RuntimeExecutionContext): Promise<JsonObject> {
    ...
    for (const transaction of transactions) {
      const transactionActions: JsonObject[] = [];
      const transactionUnresolved: string[] = [];
      for (const unitName of transaction.execution.unitNames as string[]) {
        try { const action = { type: 'unit-kill', unitName, result: await this.options.authority.killUnit(unitName, 'SIGKILL') }; actions.push(action); transactionActions.push(action); if (!await this.options.authority.verifyUnitAbsent(unitName)) transactionUnresolved.push(`unit:${unitName}`); }
        catch (error) { transactionUnresolved.push(`unit:${unitName}:${error instanceof Error ? error.message : 'unknown error'}`); }
      }
      for (const machineId of transaction.execution.allMachineIds as string[]) {
        try { const action = { type: 'machine-kill', machineId, result: await this.options.authority.killMachine(machineId) }; actions.push(action); transactionActions.push(action); if (!await this.options.authority.verifyMachineAbsent(machineId)) transactionUnresolved.push(`machine:${machineId}`); }
        catch (error) { transactionUnresolved.push(`machine:${machineId}:${error instanceof Error ? error.message : 'unknown error'}`); }
      }
      for (const jobId of transaction.execution.allJobIds as string[]) {
        try { const action = { type: 'job-kill', jobId, result: await this.options.authority.killJob(jobId, 'SIGKILL') }; actions.push(action); transactionActions.push(action); if (!await this.options.authority.verifyJobTerminal(jobId)) transactionUnresolved.push(`job:${jobId}`); }
        catch (error) { transactionUnresolved.push(`job:${jobId}:${error instanceof Error ? error.message : 'unknown error'}`); }
      }
      unresolved.push(...transactionUnresolved.map((entry) => `${transaction.transactionId}:${entry}`));
      this.options.transactions.reconcileTransition(transaction.transactionId, transactionUnresolved.length === 0 ? 'CANCEL_REQUESTED' : 'RECOVERY_REQUIRED', transactionUnresolved.length === 0 ? 'emergency_kill' : 'emergency_kill_incomplete', { scope: requestedScope, selector, unresolved: transactionUnresolved, actions: transactionActions }, `kill:${principal.principalId}`);
    }
