...
function validRequest(value:unknown):ProviderRequest {
  ...
  if(typeof input.operation!=='string'||!['describe','create','get','list','exec','stop','remove','snapshot','restore','poolReconcile','reconcile'].includes(input.operation))throw new MicrovmError('microvm_invalid_request','provider operation is invalid');
